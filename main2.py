...
def load_and_preprocess_data(data_size=6, random_state=42):                                             
    try:
        ...
        # Apply SMOTE with k_neighbors=1 to balance the training data
        smote = SMOTE(k_neighbors=1, random_state=random_state)
        ...
        visualize_class_distribution(y, y_train_balanced)
...
def visualize_class_distribution(original_y, balanced_y):
    ...
    sns.countplot(x=original_y, label="Original")
    plt.title("Class Distribution Before SMOTE")
    plt.xticks(ticks=[0, 1], labels=["Not Dry", "Likely Dry"])
    plt.show()

    plt.figure(figsize=(10, 5))
    sns.countplot(x=balanced_y, label="Balanced")
